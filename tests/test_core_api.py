...
def test_core_ml_factory():
    """Test factory instantiates BaseCoreMLClient based on CORE_ML_BACKEND."""
    from app.config import settings
    from app.core_api.real_client import RealCoreMLClient

    orig_backend = getattr(settings, "CORE_ML_BACKEND", "mock")
    try:
        settings.CORE_ML_BACKEND = "mock"
        assert isinstance(get_core_ml_client(), MockCoreMLClient)

        settings.CORE_ML_BACKEND = "real"
        assert isinstance(get_core_ml_client(), RealCoreMLClient)
    finally:
        settings.CORE_ML_BACKEND = orig_backend


def test_real_core_ml_client():
    """Test RealCoreMLClient in-process execution with real lunar images."""
    from app.core_api.real_client import RealCoreMLClient
    client = RealCoreMLClient()
    result = client.register("data/samples/verified_a.tif", "data/samples/verified_b.tif")
    assert result.status == "success"
    assert result.inliers > 100
    assert result.rmse < 0.5
    assert result.subpixel_accuracy is True
    assert result.registered_image.startswith("data:image/")
    assert result.overlay_image.startswith("data:image/")
    assert result.match_points_image.startswith("data:image/")
